...
def return_view(page_value,resident_id,return_check):
    try:
	    ...
	    door_record = door_record_value(page_value)
	    print(door_record)
	    if request.method == 'POST':
		    today = serch_today_value(page_value,resident_id,return_check)
		    ...
		    if str(request.form['door_time']) != str(door_record[3]):
			    if request.form['go_out'] == 'go':
				    if request.form['select_resident_id'].endswith('(一部)'):
					    post_door_record('exit_day','exit_time',request.form['select_resident_id'][:-10],page_value,request.form['door_time'],request.form['select_resident_id'][-10:])
				    elif request.form['select_resident_id'].endswith('可能'):
					    print(request.form['select_resident_id'][-6:])
					    post_door_record('exit_day','exit_time',request.form['select_resident_id'][:-6],page_value,request.form['door_time'],request.form['select_resident_id'][-6:])
			    elif request.form['go_out'] == 'return':
				    if request.form['select_resident_id'].endswith('(一部)'):
					    return_door = return_door_record(request.form['select_resident_id'][:-10],page_value,request.form['door_time'])
				    elif request.form['select_resident_id'].endswith('可能'):
					    return_door = return_door_record(request.form['select_resident_id'][:-6],page_value,request.form['door_time'])
				    if return_door == None:
					    if request.form['select_resident_id'][-10:] == '一人外出可能(一部)':
						    post_door_record('entrance_day','entrance_time',request.form['select_resident_id'][:-10],page_value,request.form['door_time'],request.form['select_resident_id'][-10:])
					    elif request.form['select_resident_id'][-6:] == '一人外出可能':
						    post_door_record('entrance_day','entrance_time',int(request.form['select_resident_id'][:-6]),page_value,request.form['door_time'],request.form['select_resident_id'][-6:])
				    elif return_door != None:
					    if request.form['select_resident_id'][-10:] == '一人外出可能(一部)':
						    update_door_record(page_value,request.form['door_time'],request.form['select_resident_id'][:-10])
					    elif request.form['select_resident_id'][-6:] == '一人外出可能':
						    update_door_record(page_value,request.form['door_time'],request.form['select_resident_id'][:-6])
			    
		    if request.form['select_resident_id'][-10:] == '一人外出可能(一部)':
			    today = serch_today_value(page_value,-1,return_check)
		    elif request.form['select_resident_id'][-6:] == '一人外出可能':
			    today = serch_today_value(page_value,-1,return_check)
